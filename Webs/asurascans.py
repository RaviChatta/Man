...
class AsuraScansWebs(Scraper):
    ...
    async def search(self, query: str = ""):
        ...
        # Use Cloudscraper (cs=True) for Asura Scans
        mangas = await self.get(url, headers=self.headers, cs=True)
    ...
    async def get_chapters(self, data, page: int=1):
        results = data
        # Use Cloudscraper for Asura Scans
    ...
    async def get_updates(self, page:int=1):
        ...
        while page <= 3:
            ...
            # Use Cloudscraper for Asura Scans with retries
            results = await self.get(url, headers=self.headers, cs=True, retries=3)
                ...
                if cards:
                    for card in cards:
                        try:
                            ...
                            logger.error(f"Error parsing update card: {e}")
                            continue
